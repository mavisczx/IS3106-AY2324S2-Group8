...
function SidebarLink({ name, icon, onClick, link }) {
    ...
    return <Link onClick = {onClick} to={link} >
        ...
        <Icon icon = {icon} className = "text-lg"/> <span>{name}</span> </div></span>
        </div>
        </div>
    </Link>
}
        ...
        <Link to="/">
            ...
            <div class="text-stone-100 text-xl">
                <div class="p-2.5 mt-1 flex items-center">
                    <Icon icon ="ic:baseline-people" className="text-xl rounded-md text-orange-500"/>
                    <h1 class="font-bold text-stone-200 text-[15px] ml-3">exchangemates</h1>
                </div>
